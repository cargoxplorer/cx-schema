...
### Added
- MCP workflow support (org-scoped MCP server, CXTMS-267): `McpTool`, `McpResource`, and `McpPrompt` added to the `workflowType` enum, new top-level `mcp` schema section (`name`, `title`, `description`, `uri`, `mimeType`, `timeout`) with per-type required properties, new `mcp-resource` and `mcp-prompt` templates, and `cxtms-workflow-builder` skill docs for all three types.
