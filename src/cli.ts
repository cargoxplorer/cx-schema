...
function generateModuleTemplate(name: string): string {
      ...
      - type: form
        ...
        actions:
          onSubmit:
            - type: mutation
              props:
                operation: "update${displayName.replace(/\s/g, '')}"
                input: "{{form}}"
            - type: notification
              props:
...
function runCreate(type: string | undefined, name: string | undefined): void {
  ...
  const content = type === 'module'
    ? generateModuleTemplate(safeName)
    : generateWorkflowTemplate(safeName);
